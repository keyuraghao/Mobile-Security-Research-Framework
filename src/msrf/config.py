"""Configuration for msrf.

Configuration is resolved with the following precedence (highest first):

1. Explicit keyword arguments to :func:`load_config`.
2. Environment variables prefixed ``MSRF_`` (e.g. ``MSRF_MOBSF_URL``).
3. A TOML config file (``--config`` path, ``$MSRF_CONFIG``, or the default
...
_DIRS = PlatformDirs(appname="msrf", appauthor=False)

#: Default per-user config file location (``~/.config/msrf/config.toml`` etc.).
...
class MobSFConfig(BaseSettings):
    ...
    model_config = SettingsConfigDict(env_prefix="MSRF_MOBSF_", extra="ignore")
...
class ProxyConfig(BaseSettings):
    ...
    model_config = SettingsConfigDict(env_prefix="MSRF_PROXY_", extra="ignore")
...
class FridaConfig(BaseSettings):
    ...
    model_config = SettingsConfigDict(env_prefix="MSRF_FRIDA_", extra="ignore")
...
class EmulatorConfig(BaseSettings):
    ...
    model_config = SettingsConfigDict(env_prefix="MSRF_EMULATOR_", extra="ignore")
    ...
    avd_name: str = Field(default="msrf")
...
class Config(BaseSettings):
    """Top-level msrf configuration."""

    model_config = SettingsConfigDict(env_prefix="MSRF_", extra="ignore")
...
def resolve_config_path(explicit: Path | str | None = None) -> Path | None:
    ...
    Precedence: ``explicit`` -> ``$MSRF_CONFIG`` -> default path if it exists.
    """
    if explicit:
        ...
        return path
    env_path = os.environ.get("MSRF_CONFIG")
    if env_path:
        path = Path(env_path).expanduser()
        if not path.is_file():
            raise ConfigurationError(f"$MSRF_CONFIG not found: {path}")
        return path
