"""Command-line interface for msrf.
...
def main(
    ...
    cfg.ensure_dirs()
    configure_logging(cfg.log_level, log_file=cfg.logs_dir / "msrf.log")
...
@app.command()
def version() -> None:
    """Print the msrf version."""
    console.print(f"msrf {get_version()}")


@app.command()
def info() -> None:
    """List every engine and its actions."""
    table = Table(title="msrf engines", show_lines=False)
