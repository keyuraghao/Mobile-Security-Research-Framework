...
def action(
    ...
    *,
    ...
    def decorator(func: Callable[..., Any]) -> Callable[..., Any]:
        func._msrf_action = ActionSpec(  # type: ignore[attr-defined]
...
class Engine:
    """Base class for all msrf engines.

    Subclasses set :attr:`name` and :attr:`summary`, take the shared
    :class:`~msrf.config.Config` in ``__init__`` and implement operations as
    ...
    def actions(cls) -> Iterator[ActionSpec]:
        ...
        for _, member in inspect.getmembers(cls, predicate=inspect.isfunction):
            spec = getattr(member, "_msrf_action", None)
