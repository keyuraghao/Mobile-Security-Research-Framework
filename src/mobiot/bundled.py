...
A standalone mobiot build ships its heavy dependencies inside the executable
...
When not frozen, ``MOBIOT_VENDOR_DIR`` may point at a vendor tree for testing.
"""
...
def vendor_root() -> Path | None:
    ...
    Resolution order: an explicit ``MOBIOT_VENDOR_DIR`` (for testing), then the
    PyInstaller extraction dir (``sys._MEIPASS/vendor``), then a ``vendor/``
    folder next to the executable.
    """
    env = os.environ.get("MOBIOT_VENDOR_DIR")
    if env:
        p = Path(env)
