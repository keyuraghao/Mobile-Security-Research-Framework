...
def get_logger(name: str = "mobiot") -> logging.Logger:
    """Return a namespaced logger under the ``mobiot`` root logger."""
    if name == "mobiot" or name.startswith("mobiot."):
        return logging.getLogger(name)
    return logging.getLogger(f"mobiot.{name}")
...
def configure_logging(
    ...
    *,
    ...
    """Configure the ``mobiot`` root logger exactly once.
