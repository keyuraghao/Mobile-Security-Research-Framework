...
def _md(findings, meta, out: Path) -> None:
    lines = [
        ...
        "",
        f"Generated: {meta.get('generated')}  ·  Tool: mobiot {meta.get('tool_version', '')}",
        "",
        "## Summary",
        "",
    ]
...
def _html(findings, meta, out: Path) -> None:
    ...
    out.write_text(
        f"""<!doctype html><html><head><meta charset="utf-8"><title>mobiot report</title>
<style>
...
<p>Generated: {html.escape(meta.get('generated',''))} &middot; Tool: mobiot {html.escape(str(meta.get('tool_version','')))}</p>
<p>{summary}</p>
...
def _pdf(findings, meta, out: Path) -> None:
    ...
    pdf.cell(0, 6, f"Generated: {meta.get('generated')}   Tool: mobiot {meta.get('tool_version','')}",
