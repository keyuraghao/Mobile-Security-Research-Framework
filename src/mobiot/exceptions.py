"""Exception hierarchy for mobiot.

All errors raised deliberately by mobiot derive from :class:`MobiotError` so
...
class MobiotError(Exception):
    """Base class for every error raised by mobiot."""


class ConfigurationError(MobiotError):
    """Raised when configuration is missing, malformed, or inconsistent."""


class ToolNotFoundError(MobiotError):
...
class CommandError(MobiotError):
