; Inno Setup script - builds msrf-<version>-setup.exe from the PyInstaller onedir output.
; Invoked (from the repo root) as:
;   iscc /DVersion=0.3.3 /DSource=dist\msrf packaging\installer.iss
#ifndef Version
  #define Version "0.0.0"
#endif
#ifndef Source
  #define Source "dist\msrf"
#endif
...
SetupIconFile=src\msrf\data\icon.ico
LicenseFile=LICENSE
...
Name: "addtopath"; Description: "Add the msrf command-line tool to PATH (for the CLI and MCP clients)"; GroupDescription: "Command line:"
...
begin
  ...
  begin
    Answer := MsgBox('Also remove your msrf workspace (reports, captures, logs, ' +
                     'downloaded emulator/SDK and config)?' + #13#10 + #13#10 +
                     'Choose No to keep it for a later reinstall.',
                     mbConfirmation, MB_YESNO or MB_DEFBUTTON2);
    if Answer = IDYES then
    begin
      DelTree(ExpandConstant('{localappdata}\msrf'), True, True, True);
      DelTree(ExpandConstant('{userappdata}\msrf'), True, True, True);
    end;
  end;
end;
